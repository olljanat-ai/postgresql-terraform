...
locals {
  ...
  # more. It disappears together with the password when owner_login is off.
  owner_secret = var.owner_login ? {
    owner = {
  ...
  key_vault_secrets = merge(local.owner_secret, local.administrator_secret)

  key_vault_secret_values = merge(
    var.owner_login ? { owner = random_password.owner[0].result } : {},
